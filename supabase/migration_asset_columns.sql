...
-- Finnhub/TradingView/EODHD). Si une de ces colonnes manque, l'insertion de
-- l'actif échoue (« Could not find the '…' column of 'assets' »), alors qu'un
-- ré-import Fortuneo qui réutilise des actifs déjà présents fonctionne.
...
alter table public.assets add column if not exists eodhd_symbol        text;
